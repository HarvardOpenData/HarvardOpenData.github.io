...
export const query = graphql`
  query PeoplePageQuery {
    contributors: allSanityPerson(
      filter: { position: { title: { eq: "Contributor" } } }
    ...
    faculty: allSanityPerson(
      filter: { position: { group: { eq: "Mentors" } } }
    ) {
      edges {
        node {
          id
    ...
    founder: allSanityPerson(
      filter: { position: { title: { eq: "Founder" } } }
    ) {
      edges {
        node {
          id
    ...
    boardEmeritus: allSanityPerson(
      filter: { position: { title: { eq: "Board Emeritus" } } }
      sort: { fields: [name], order: ASC }
    ) {
      edges {
        node {
...
const PeoplePage = (props) => {
  ...
  const boardNodes = data && data.board && data.board.edges[0].node.members;
  ...
  function filterAlumni(allContributors) {
    ...
    if (month > 4) {
      currentContributors = allContributors.filter(
        (contributor) => contributor.year > year
      );
      alumni = allContributors.filter(
        (contributor) => contributor.year <= year
      );
    } else {
      currentContributors = allContributors.filter(
